...
public class APathFinder {
	...
	private Node start, end, par;
	private int size, diagonalCost;
	...
	public APathFinder(Grid grid, int size) {
		this.grid = grid;
		this.size = size;
	...
	public APathFinder(Grid grid, int size, Node start, Node end) {
		this.grid = grid;
		this.size = size;
		this.start = start;
		this.end = end;
	...
	public Node getPar() { return par; }
	public boolean isNoPath() { return noPath; }
	public boolean isDiagonal() { return diagonal; }
	public boolean isTrig() { return trig; }
	public void setDiagonal(boolean diagonal) { this.diagonal = diagonal; }
	public void setTrig(boolean trig) { this.trig = trig; }
	public void addBorder(Node node) { if(!checkBorderDuplicate(node)) borders.add(node); }
	public void addOpen(Node node) { if(!checkOpenDuplicate(node)) open.add(node); }
	public void addClosed(Node node) { if(!checkClosedDuplicate(node)) closed.add(node); }
	public void addPath(Node node) { path.add(node); }
	public void removePath(int location) { path.remove(location); }
	public void removeBorder(int location) { borders.remove(location); }
	public void removeClosed(int location) { closed.remove(location); }
	public void removeOpen(int location) { open.remove(location); }
	public ArrayList<Node> getBorderList() { return borders; }
	public ArrayList<Node> getClosedList() { return closed; }
	public ArrayList<Node> getPathList() { return path; }
	public ArrayList<Node> getOpenList() { return open; }
	public Node getOpen(int location) { return open.get(location); }
	...
	public void calculateNodeValues(int possibleX, int possibleY, Node openNode, Node parent) {
		...
		if (possibleX < 0 | possibleY < 0 | possibleX >= grid.getWidth() | possibleY >= grid.getHeight()) {
			return;
		}
		...
		// Calculating H Cost
		int HxDiff = Math.abs(end.getX() - openNode.getX());
		int HyDiff = Math.abs(end.getY() - openNode.getY());
	...
	public void connectPath() {
		if (getPathList().size() == 0) {
			Node parentNode = end.getParent();

			while (!Node.isEqual(parentNode, start)) {
				addPath(parentNode);
	...
	public void findPath(Node parent) {
		...
		if (Node.isEqual(parent, end)) {
			end.setParent(parent.getParent());
